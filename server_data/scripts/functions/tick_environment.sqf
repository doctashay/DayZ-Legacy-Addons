private["_agent","_pos","_isWater","_bones","_delta","_anim","_bodyPosition","_isSwim","_playerWet","_isDrying"];
_agent = _this;
...
_airTemperature = 9; //airTemperature;
			...
			*/
			...
			_inside = itemsInCargo _item;
			{
				...
				if(_cargoitem isKindOf 'ClothingBase' and _waterproof > 0 and (_itemwet == 0 or !_isDrying))then{
					...
					if(!(_isWater) and ((damage _cargoitem) < 1) and isOn _cargoitem)then{
						_temper = getTemperature _cargoitem;
						if(_cargoitem isKindOf 'Tool_Heatpack')then{ 
							_cargoitem setDamage ((damage _cargoitem)+0.01);
							if(_temper > 0)then{
								_cargoitem setTemperature (_temper-0.01);
								_heatpackon = _heatpackon + (_temper/2);
							};
						};
						if (_cargoitem isKindOf 'Food_Guts')then{
							if(_temper > 0)then{
								_cargoitem setTemperature (_temper-0.02);
								_heatpackon = _heatpackon + (_temper/3);
							};
						};
					};
				};
...
_heatcomfort = (_heatpackon+(((_airTemperature+_totalHeatIsolation) max 7)*(((_agentSpeed/3.2) max 1) - _playerWet))- ( _playerTemperature + (((worldLightScale max 0) min 2)*10) - ((windSpeed*3) min 6) - ((getPosASL _agent select 2)/100) ) ) min 100;
