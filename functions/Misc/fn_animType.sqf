...
#define COMPOSE_NAME(ID,NAME)\
	if (_includeType && _typeName != "N/A" && !_ignoreIncludeType) then {_typeName = format ["[%1] %2",_type,_typeName];};\
...
_animType = if (_includeCategory) then {
	[["Action","N/A"],["Pose","N/A"],["Movement","N/A"],["Stance","N/A"],["Hand item","N/A"],["Direction","N/A"]]
} else {
	...
	switch _prefix do {
		...
		case "A": {
			_typeName = switch _type do {
				...
				case "exc": {"Excretion"};
				case "tck": {"Attack"};
				default {_ignoreIncludeType = true; "N/A"};
			};
		...
		case "P": {
			_typeName = switch _type do {
				...
				case "cro": {"Crouch"};
				case "001": {_ignoreIncludeType = true; "N/A (CargoActions)"}; //--- Anim viewer exception
				case "002": {_ignoreIncludeType = true; "N/A (DeadActions)"}; //--- Anim viewer exception
				case "003": {_ignoreIncludeType = true; "N/A (ParachuteFreeFall)"}; //--- Anim viewer exception
