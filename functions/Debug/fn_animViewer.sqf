...
if (count _this <= 1) exitwith {
	with uinamespace do {
		...
		) exitwith {"Animation Viewer is already running" call bis_fnc_log;};
...
_fnc_loadCombo = {
	...
	if (count _nextTypes == 0) then {_nextTypes = ["N/A"];};
...
switch _mode do {
	...
	case "Init": {
		...
		BIS_fnc_animViewer_unit = _objnull;
	...
	case "Exit": {
		...
		deletevehicle BIS_fnc_animViewer_logic;
		deletevehicle BIS_fnc_animViewer_target;

		setviewdistance -1;
		_cam = BIS_fnc_animViewer_cam;
	...
	case "Soldier": {
		...
		if (count _actions == 0) then {_actions = ["N/A"];};
		...
		//--- Unit preview
		deletevehicle BIS_fnc_animViewer_unit;
		_logic = BIS_fnc_animViewer_logic;
		#ifdef DZ
	...
	case "Mouse": {
		...
		_cam = BIS_fnc_animViewer_cam;
		_logic = BIS_fnc_animViewer_logic;
		_target = BIS_fnc_animViewer_target;
