...
#define INIT			_display = BIS_fnc_configviewer_display;
...
switch _mode do {
	...
	case "Init": {
		...
		if (typename BIS_fnc_configviewer_root == typename false) then {
			...
			_lbAdd = GUI_BOOKMARKS lbadd "<Bookmark selected class>"; //--- ToDo: Localize
			GUI_BOOKMARKS lbsetpicture [_lbadd,"\DZ\ui\data\gui\rsc\rscdisplayconfigviewer\bookmark_gs.paa"];
			{
				_path = _x select 0;
	...
	case "lbSelChanged": {
		...
		if (_lbData == BIS_fnc_configviewer_selected) exitwith {};
		BIS_fnc_configviewer_selected = _lbData;
		...
		GUI_PREVIEWCLASS ctrlsettext format ["Create %1",_class]; //--- ToDO: Localize
					...
					if (_isClasses) then {
						...
						if (_shift) then {
							...
							for "_i" from 0 to (lbsize GUI_PARAMS - 1) do {
								_text = _text + (GUI_PARAMS lbtext _i) + "
"; //--- Line break
							};
	...
	case "paramLbSelChanged": {
		...
		GUI_PREVIEWPARAM ctrlsettext format ["Preview %1",_lbData]; //--- ToDO: Localize
	};
