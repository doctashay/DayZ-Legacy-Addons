...
{	
	...
	case 1:
	{
		...
		//burn
		while {_fuel >= _fuel_burn_rate && (isOn _gascooker)} do
		{
			sleep _update_interval; 
			
			//temperature increase
			_temperature = _gascooker getVariable ['temperature', 20];
	...
	{
		_gascooker = _params select 0;
		_temperature = _gascooker getVariable 'temperature';
		_check_cooking = true;
		
		while {_temperature >= _min_cooker_temp && !(isOn _gascooker)} do
		{
			sleep _update_interval;
