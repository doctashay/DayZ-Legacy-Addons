...
// 				_arrayElements = count (_cfgLoc select _indexLoc);

// 				_position = [0,0];
// 				_angle = random( 360 );

// 				if ( _arrayElements < 3 ) then
// 				{
// 					_position = _cfgLoc select _indexLoc;
// 				}
// 				else
// 				{
// 					_position set [_position select 0, (_cfgLoc select _indexLoc) select 0];
// 					_position set [_position select 1, (_cfgLoc select _indexLoc) select 1];
// 					_angle = (_cfgLoc select _indexLoc) select 2;
// 				};
...
// 					_text = format["Event Group: %4, Server Event: %1, Spawned At: %2 Angle: %5, Vehicles No.: %3", _event, _position, _s, _x, _angle];
// 					diag_log _text;
